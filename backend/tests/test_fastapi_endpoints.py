...
class TestFastAPIEndpoints:
    ...
    def test_query_endpoint_with_real_system(self, client):
        ...
        print(f"Query response body: {response.json() if response.status_code == 200 else response.text}")
        
        if response.status_code != 200:
            pytest.fail(f"Query endpoint failed with status {response.status_code}: {response.text}")
    ...
    def test_courses_endpoint(self, client):
        ...
        print(f"Courses response: {response.json() if response.status_code == 200 else response.text}")
    ...
    def test_query_endpoint_with_mocked_rag_system_error(self, mock_rag_system, client):
        ...
        if "query failed" in response.text.lower():
            pytest.fail("Found 'query failed' in error response - this might be the issue!")
    ...
    def test_cors_headers(self, client):
        ...
        response = client.post("/api/query", json={"query": "test"})
        print(f"CORS headers in response: {response.headers.get('access-control-allow-origin')}")
    ...
    def test_request_validation(self, client):
        ...
        test_cases = [
            ...
            {"data": {"query": "test", "session_id": 123}, "description": "Non-string session_id"},
        ]
        
        for case in test_cases:
            response = client.post("/api/query", json=case["data"])
            print(f"{case['description']} - Status: {response.status_code}, Response: {response.text}")
            ...
            if response.status_code == 422:
                ...
                pytest.fail(f"Got 'query failed' for {case['description']} - validation issue!")
...
class TestFrontendBackendIntegration:
    ...
    def test_javascript_can_call_api(self, client):
        ...
        print(f"Frontend-style request response: {response.json() if response.status_code == 200 else response.text}")
    ...
    def test_concurrent_requests(self, client):
        ...
        def make_request(query_num):
            response = client.post("/api/query", json={"query": f"What is lesson {query_num}?"})
