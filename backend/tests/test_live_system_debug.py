...
class TestLiveSystemDebug:
    ...
    def test_ai_generator_tool_calling_mechanism(self, real_config):
        ...
        with patch('ai_generator.anthropic.Anthropic') as mock_anthropic_class:
            ...
            ai_generator = AIGenerator(real_config.ANTHROPIC_API_KEY, real_config.ANTHROPIC_MODEL)
    ...
    def test_full_system_integration_with_mocked_externals(self, real_config):
        ...
        with patch('vector_store.chromadb.PersistentClient') as mock_chroma, \
            ...
            mock_final_response = Mock()
            mock_final_response.content = [Mock()]
            mock_final_response.content[0].text = "MCP is a framework that allows you to build AI applications with rich context."
            
            anthropic_client.messages.create.side_effect = [mock_tool_response, mock_final_response]
            ...
            assert anthropic_client.messages.create.call_count == 2  # Tool call + final response
    ...
    def test_system_with_no_courses_loaded(self, real_config):
        ...
        with patch('vector_store.chromadb.PersistentClient') as mock_chroma, \
            ...
            mock_final_response = Mock()
            mock_final_response.content = [Mock()]
            mock_final_response.content[0].text = "I couldn't find any information about MCP in the course materials."
