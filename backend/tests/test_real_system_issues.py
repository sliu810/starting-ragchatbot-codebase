...
class TestRealSystemIssues:
    ...
    def test_vector_store_has_data(self):
        ...
        assert course_count > 0, "No courses found in vector store - this could be the issue!"
    ...
    def test_search_tool_with_real_data(self):
        ...
        assert "no relevant content found" not in result.lower() or len(search_tool.last_sources) == 0
    ...
    def test_outline_tool_with_real_data(self):
        ...
        assert "not found" not in result.lower(), f"Outline tool couldn't find course: {result}"
        assert "**" in result, "Outline should contain formatted course title"
    
    @pytest.mark.skipif(not config.ANTHROPIC_API_KEY or config.ANTHROPIC_API_KEY == "your-api-key", 
    ...
    def test_system_prompt_format(self):
        ...
        assert "get_course_outline" in system_prompt, "System prompt missing outline tool reference"
        assert "search_course_content" in system_prompt, "System prompt missing search tool reference"
        assert "Tool Usage Guidelines" in system_prompt, "System prompt missing tool usage section"
...
class TestEnvironmentSetup:
    ...
    def test_api_key_configured(self):
        ...
        print(f"API Key configured: {'Yes' if config.ANTHROPIC_API_KEY and config.ANTHROPIC_API_KEY != 'your-api-key' else 'No'}")
        print(f"API Key starts with: {config.ANTHROPIC_API_KEY[:10] if config.ANTHROPIC_API_KEY else 'None'}...")
        
        if not config.ANTHROPIC_API_KEY or config.ANTHROPIC_API_KEY == "your-api-key":
            pytest.skip("No valid API key configured - this could be the issue with 'query failed'")
    ...
    def test_config_values(self):
        ...
        print(f"Has API key: {'Yes' if config.ANTHROPIC_API_KEY and len(config.ANTHROPIC_API_KEY) > 10 else 'No'}")